...
case class RequestSelectionModel(
    requestedAmounts: SortedMap[Item, Double],
    search: SearchQuery,
    requestAmountEditor: Option[( Item, InputModel )]
):
  ...
  private def editRequestAmountStart[F[_]: Async]( item: Item ): ( RequestSelectionModel, Cmd[F, Nothing] ) =
    requestedAmounts
      .get( item )
