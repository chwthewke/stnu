...
object BrowseView:
    ...
    override def displayItem( env: Env )( item: Item ): List[Html[Nothing]] =
      List(
        ...
        Html.td(
          ...
          Html.span( b.mx1 + b.tag + b.isPrimary )( show"Tier ${item.tier}" )
        )
      )
  ...
  object SearchableRecipeTable extends SearchableTable[Recipe.NonExtraction, BrowseMsg]:
    ...
    override def searchTargets( recipe: Recipe.NonExtraction ): Vector[String] =
      recipe.displayName +: recipe.items.map( _.displayName ).toVector
    ...
    private def displayRecipe[R <: Recipe, M <: BrowseMsg]( env: Env, recipe: R )(
        displayProducts: recipe.P[Countable[Double, Item]] => Html[M]
    ): List[Html[BrowseMsg]] =
      List(
