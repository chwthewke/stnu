...
case class ContentModel(
  ...
  lazy val env: Env =
    ...
    val transportIcons: Map[ClassName[Transport], Uri] =
      ( model.game.conveyorBelts ++: model.game.pipelines ).toVector
        .mapFilter: transport =>
          model.icons.getIconPath( transport.className ).map( links.image ).tupleLeft( transport.className )
        .toMap

    Env( model.game, itemIcons, machineIcons, transportIcons )
