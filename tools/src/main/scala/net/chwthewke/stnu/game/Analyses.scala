...
case class Analyses[A](
  ...
  def keys: Vector[ClassName[Any]] =
    ( schematics.keys ++ items.keys ++ recipes.keys ++ powerGenerators.keys ++ extractors.keys ).toVector
